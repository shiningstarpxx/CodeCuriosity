...
class Solution(object):
    def latestTimeCatchTheBus(self, buses, passengers, capacity):
        ...
        pos = 0
        left = 0 # 没有这个声明，也能通过。看上去，python对于变量的生命周期管理很奇怪，和其他语言不一样
        # 模拟
        for bus in buses:
            left = capacity
            while pos < len(passengers) and passengers[pos] <= bus and left > 0:
                pos += 1
                left -= 1

        pos -= 1
        last_mini = buses[-1] if left > 0 else passengers[pos]
        while pos >= 0 and passengers[pos] == last_mini:
            pos -= 1
            last_mini -= 1
        return last_mini
